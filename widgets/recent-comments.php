...
class Lovecraft_Recent_Comments extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'Widget_Lovecraft_Recent_Comments', __( 'Recent Comments', 'lovecraft' ), $widget_ops );
	}
				...
				if ( $comments ) {
					foreach ( $comments as $comment ) { ?>
						...
						<li>
							...
							<a href="<?php echo get_permalink( $comment->comment_post_ID ); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php printf( _x( 'Comment to %1$s, posted %2$s', 'Translators: %1$s = post title, %2$s = post date', 'lovecraft' ), get_the_title( $comment->comment_post_ID ), get_the_time( get_option( 'date_format' ) ) ); ?>">
